...
def annotate_genes_mygene(
    ...
    # Build a mapping from gene_identifiers to the actual var index
    gene_to_idx = {}
    if gene_col is None:
        # When using index, the mapping is direct
        for i, idx in enumerate(adata.var.index):
            gene_to_idx[str(idx)] = idx
    else:
        # When using a column, we need to map from column values to index
        for i, (idx, gene_id) in enumerate(zip(adata.var.index, adata.var[gene_col])):
            if gene_id is not None and not pd.isna(gene_id):
                gene_to_idx[str(gene_id)] = idx
    ...
    # Map from clean_id back to original index
    clean_to_idx = {}
    for orig_id, clean_id in original_to_clean.items():
        if orig_id in gene_to_idx and clean_id:
            clean_to_idx[clean_id] = gene_to_idx[orig_id]
    ...
    missing_indices = var.index[missing]
    
    # Get corresponding clean IDs for missing coordinates
    genes_to_query = []
    for i, is_missing in enumerate(missing):
        if is_missing and i < len(clean_identifiers):
            clean_id = clean_identifiers[i]
            if clean_id:
                genes_to_query.append(clean_id)
    ...
    if genes_to_query:
        ...
        # Update var with coordinates using the index mapping
        coordinates_added = 0
        for clean_id, (chrom, start, end) in gene_to_coords.items():
            if clean_id in clean_to_idx:
                idx = clean_to_idx[clean_id]
                var.at[idx, 'chromosome'] = chrom
                var.at[idx, 'start'] = start
                var.at[idx, 'end'] = end
                coordinates_added += 1
        
        print(f"Added coordinates for {coordinates_added} genes")
    ...
    if drop_missing:
        ...
        # Handle case where no genes have coordinates
        if var_filtered.shape[0] == 0:
            print("Warning: No genes with coordinates found. Returning original AnnData.")
            return adata
